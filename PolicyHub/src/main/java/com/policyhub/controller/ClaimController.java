...
public class ClaimController {
	...
	public ResponseEntity<?> getByCustomer(@PathVariable Integer customerId, HttpSession session) {
		User u = (User) session.getAttribute("loggedInUser");
		if (u == null)
	...
	public ResponseEntity<?> updateStatus(@PathVariable Integer claimId, @RequestParam ClaimStatus status,
			HttpSession session) {
		User u = (User) session.getAttribute("loggedInUser");
